  ...
  return (
    <div className="bg-[#050510] blue:bg-[#050510] dark:bg-[#0A0A0A] light:bg-[#F8F9FA] pt-32 transition-colors duration-300">
      <section className="py-20 px-6 lg:px-12 max-w-[1800px] mx-auto">
        <h1 className="text-5xl md:text-7xl font-medium text-white blue:text-white dark:text-white light:text-black mb-16 transition-colors">Our Expertise</h1>
            ...
            >
              <div className="flex items-center gap-4 mb-6">
                <motion.div 
                  ...
                  className="w-12 h-12 rounded-xl bg-white/10 blue:bg-white/10 dark:bg-white/10 light:bg-black/10 flex items-center justify-center group-hover:bg-white/20 blue:group-hover:bg-white/20 dark:group-hover:bg-white/20 light:group-hover:bg-black/20 transition-colors"
                >
                  <item.icon className="text-white blue:text-white dark:text-white light:text-black group-hover:scale-110 transition-transform transition-colors" size={24} />
                </motion.div>
                <span className="text-white/20 blue:text-white/20 dark:text-white/20 light:text-black/20 font-mono transition-colors">{item.id}</span>
              </div>
              <h3 className="text-2xl font-medium text-white blue:text-white dark:text-white light:text-black mb-4 transition-colors">{item.title}</h3>
              <p className="text-white/60 blue:text-white/60 dark:text-white/60 light:text-black/60 mb-8 leading-relaxed transition-colors">{item.desc}</p>
              <div className="space-y-3">
                <p className="text-sm font-medium text-white/40 blue:text-white/40 dark:text-white/40 light:text-black/40 uppercase tracking-wider transition-colors">Includes:</p>
                {item.includes.map((inc, j) => (
                  <div key={j} className="flex items-center gap-3 text-white/80 blue:text-white/80 dark:text-white/80 light:text-black/80 transition-colors">
                    <div className="w-1.5 h-1.5 rounded-full bg-white/30 blue:bg-white/30 dark:bg-white/30 light:bg-black/30 transition-colors" />
                    {inc}
                  </div>
                ))}
              </div>
      ...
      <section className="py-24 bg-white/5 blue:bg-white/5 dark:bg-white/5 light:bg-black/5 transition-colors">
        <div className="max-w-[1800px] mx-auto px-6 lg:px-12">
          <h2 className="text-3xl font-medium text-white blue:text-white dark:text-white light:text-black mb-12 text-center transition-colors">Industries We Serve</h2>
          <div className="flex flex-wrap justify-center gap-4">
            {industries.map((ind, i) => (
              <span key={i} className="px-6 py-3 rounded-full bg-[#050510] blue:bg-[#050510] dark:bg-[#0A0A0A] light:bg-white border border-white/10 blue:border-white/10 dark:border-white/10 light:border-black/10 text-white/80 blue:text-white/80 dark:text-white/80 light:text-black/80 hover:border-white/40 blue:hover:border-white/40 dark:hover:border-white/40 light:hover:border-black/40 transition-colors cursor-default transition-colors">
                {ind}
              </span>
            ))}
          </div>
